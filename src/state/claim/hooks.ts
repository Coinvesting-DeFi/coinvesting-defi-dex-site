import { COINVEX } from './../../constants/index'
import { TokenAmount, JSBI, ChainId } from '@coinvestingdefi/dex-sdk'
...
// check if user is in blob and has not yet claimed COINVEX
...
export function useUserUnclaimedAmount(account: string | null | undefined): TokenAmount | undefined {
  ...
  if (!canClaim || !userClaimData) {
    return new TokenAmount(coinvex, JSBI.BigInt(0))
  }
  return new TokenAmount(coinvex, JSBI.BigInt(userClaimData.amount))
}
      ...
      return distributorContract
        ...
        .then((response: TransactionResponse) => {
          addTransaction(response, {
            summary: `Claimed ${unClaimedAmount?.toSignificant(4)} COINVEX`,
